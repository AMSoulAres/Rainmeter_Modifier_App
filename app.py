...
path = r'C:/Users/henri\Documents/Rainmeter/Skins/Ageo/Time and Date/Time and Date.ini'
...
def modify_files(filename, section, key, value):
    if os.path.isfile(filename):
        parser = ConfigParserMultiOpt()
        try:
            parser.read(filename, encoding='utf-16')
        except Exception as e:
            parser.read(filename, encoding='utf-8')
            print(e)
        parser.set(section, key, value)
        cfgfile = open(filename, 'w', encoding='utf-16')
        parser.write(cfgfile, space_around_delimiters=False)
        cfgfile.close()
    else:
        print("Config file not found")


def save_prev_rgb(filename, section, key):
    if os.path.isfile(filename):
        parser = ConfigParserMultiOpt()
        try:
            parser.read(filename, encoding='utf-16')
            ...
            parser.read(filename, encoding='utf-8')
        rgb = parser.get(section, key)
        cfgfile = open(filename, 'r', encoding='utf-16')
        cfgfile.close()
        print(rgb)
        return rgb
